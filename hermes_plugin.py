...
def _kind(error):
    if isinstance(error, dict):
        ...
        value = type(error).__name__ if error else ''
...
def _save(ctx, session_id, task_id, failure):
    ...
    rows.append({'key': key, 'session_id': session_id, 'task_id': str(task_id or ''),
                 'failure': failure, 'count': 1, 'first_seen': int(time.time()),
                 'last_seen': int(time.time()), 'status': 'open'})
    ctx.state.set('incidents', rows[-MAX_INCIDENTS:])
...
def register(ctx):
    def api_error(*, session_id='', task_id='', error=None, retryable=None,
        ...
        # Intermediate retry failures are telemetry, not incidents.
